...
public:
    explicit CubicCongestionControl(const CubicCongestionCtlConfig& ccConfig)
    {
        ...
        m_c = ccConfig.c;
        m_beta = ccConfig.beta;
        m_w_max = ccConfig.w_max;
        m_last_max_cwnd = ccConfig.last_max_cwnd;
...
private:
    ...
    void OnDataRecv(const AckEvent& ackEvent)
    {
      ...
      const auto acked_bytes = ackEvent.ackPacket;
      ...
      m_cwnd = m_c * pow(m_time - m_K, 3.0) + m_w_max;
    }
    ...
    void OnDataLoss(const LossEvent& lossEvent)
    {
        ...
        const uint32_t curr_time_ms = maxsentTic ;
        ...
        else
        {
            ...
            const uint32_t delta_cwnd = std::min(uint32_t(m_K * std::pow(curr_time_ms, 3)), w_max - m_cwnd);
    ...
    uint32_t m_c = 0.4 ; //cubic function constant
    uint32_t m_time= 0 ; // time since last congestion event
    uint32_t m_K = 0.1 ; // inflection point constant
    uint32_t m_beta;
    uint32_t m_w_max = 65535.0; //max window size
    uint32_t m_last_max_cwnd;
