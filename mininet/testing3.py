...
server_num = 16  # data nodes number
client_num = 2 
...
if __name__ == '__main__':
    ...
    # 2 clients
    clients_nodes = []
    info(f'****Adding {client_num} clients nodes')
    for i in range(client_num):
        c = net.addHost(name=f'client{i+1}', ip=f'10.100.0.{i+1}')
        clients_nodes.append(c)
    
    # EX: clients_nodes[1] = net.addHost(name='client', ip='10.100.0.1')
    ...
    # TCLink for bandwidth and loss rate
    ...
    for i in range(client_num):
        net.addLink(clients_nodes[i], leftswitch, cls=TCLink, bw=100)  # same bandwidth

    serverdelays = ['20ms', '30ms', '40ms', '25ms', '30ms', '35ms', '40ms', '45ms', '50ms', '55ms', '20ms', '30ms', '40ms', '25ms', '30ms', '35ms', \
                    '20ms', '30ms', '40ms', '25ms', '30ms', '35ms', '40ms', '45ms', '50ms', '55ms', '20ms', '30ms', '40ms', '25ms', '30ms', '35ms']
    serverbws = [5, 5, 100, 10, 10, 10, 10, 10, 10, 10, 5, 5, 100, 10, 10, 10, \
                5, 5, 100, 10, 10, 10, 10, 10, 10, 10, 5, 5, 100, 10, 10, 10]  # Mbits
  # Mbits
    ...
    # automatically run the clients:
    # client_proc_ls = []
    # for i in range(client_num):
    #     client_start_cmd = f'{curr_dir}/MPDtest{i+1} {curr_dir}/config/downnode_mn-16_{i+1}.json '
    #     client_start_cmd_args = shlex.split(client_start_cmd)
    #     # clients_nodes[i].cmd(client_start_cmd_args)
    #     client_proc = clients_nodes[i].popen(client_start_cmd_args)
    #     client_proc_ls.append(client_proc)
    # sleep(1)
